...
class ExperimentConfig:
    ...
    gamma: float = MISSING
    lr: float = MISSING  # Global fallback learning rate
    actor_lr: Optional[float] = MISSING  # Optional override for actor/policy networks
    critic_lr: Optional[float] = MISSING  # Optional override for critic/value/Q networks
    alpha_lr: Optional[float] = MISSING  # Optional override for temperature / entropy coefficient (e.g., SAC)
...
class Experiment(CallbackNotifier):
        ...
        self.optimizers = {
            group: {
                loss_name: torch.optim.Adam(
                    params,
                    lr=self._get_group_lr(loss_name),
    ...
    def _get_group_lr(self, loss_name: str) -> float:
        actor_names = {"loss_objective", "loss_actor"}
        critic_names = {"loss_critic", "loss_value", "loss_qvalue"}
        if loss_name in actor_names and self.config.actor_lr is not None:
            return self.config.actor_lr
        if loss_name in critic_names and self.config.critic_lr is not None:
            return self.config.critic_lr
        if loss_name == "loss_alpha" and self.config.alpha_lr is not None:
            return self.config.alpha_lr
